...
export const runExec = async () => {
    ...
    const projectName = [];
    try {
        const CmakeFile = await fs.readFileSync(path.resolve(String(`${projectPathName}/CMakeLists.txt`)), "utf-8");
        if (CmakeFile) {
    ...
    if (!projectName || projectName.length !== 1) {
        console.log("error: couldn't find project name in CMakeLists.txt file.");
        return;
    }
    if (platform == "win32") {
        try {
            const execFilePath = path.resolve(`./build/Debug/${projectName[0]}.exe`);
            ...
            console.log(`error: make sure your cmake project name is the same as your baseDir (${projectName[0]}`);
        }
    }
...
export const runBuild = () => {
    exec("cmake -S . -B ./build && cd ./build && make", (error, stdout, stderr) => {
        if (error) {
            console.log(`error: ${error.message}`);
            return;
        }
